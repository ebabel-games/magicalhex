define(['constants', 'round', 'zone'], (C, round, Zone) => {
  ...
  const updateCurrentZone = (currentZone, scene, camera, loadedZones) => {
    if (!currentZone) {
      ...
      // This name is pushed because there was no current zone at all,
      // so this is a brand new zone just created.
      loadedZones.push(currentZone.name);
    ...
    // Note: that zone should already have been pre loaded into the game, so it
    // should not create its meshes again.
