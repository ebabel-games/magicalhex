...
const render = function render (input) {
    ...
    // Mob actions run from here, once per actionsTimer.
    // Display all mob names: 
    // input.mob.children.map(function (_mob) { return _mob.userData.targetName });
    input.actionsTimer = input.actionsTimer + delta;
    if (input.actionsTimer > 0.25) {
